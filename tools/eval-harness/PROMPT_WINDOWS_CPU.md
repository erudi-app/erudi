# Prompt for the coding agent (Windows CPU machine)
...
You are running a memory and storage evaluation of the **installed** Erudi desktop app (CPU build) on this Windows machine, with a harness that was written and tested on macOS only. Your job: make it run correctly on Windows, run it, and bring back the results. Answer me in French; keep code and code comments in English. Nothing here is committed anywhere: work in this folder only.
...
- Windows-specific parts were written from the Erudi source code and have **never run on Windows**: install/data/log paths, process classification (`Erudi.exe` helpers with `--type=`, `backend.exe`, `postgres.exe` under `pginstall\bin`, `llama-server.exe`), the exe version query, `taskkill /PID` quit, the storage walker's logical sizes. Expect to fix some of them.
- The Erudi app must be the **CPU** build (the installer without `-cuda`), version 1.1.2, installed per-user (default `%LOCALAPPDATA%\Programs\Erudi\Erudi.exe`). If it is not installed or not 1.1.2, stop and tell me.
- Every run of this prompt uses the flavour **`win-cpu`**: pass `--flavour win-cpu` so the harness refuses to measure if the installed app turns out to be the CUDA build (it asks the running app for its `backend_type` and engine). The flavour decides the model (`Qwen/Qwen3-4B-GGUF`), the expected inference process (`llama-server.exe` from `...\artifacts\llama-cpp\cpu\bin`), that **no GPU memory is measured or expected**, and that `--threads` and `-ngl` (which must be `0` on this build) are recorded from the child's command line. Its definition is `flavours/win-cpu.json`.
- This is the interesting build for a small machine: everything the model needs sits in ordinary process memory, so the inference figure and the app overhead land in the same budget. Expect a slow first turn and a much lower throughput than a GPU build; that is the measurement, not a defect.
...
2. `uv run erudi_eval.py selftest`. Check that it finds the install, the data root (`%LOCALAPPDATA%\erudi\backend\prod`), the stdout capture log (`%TEMP%\erudi-backend.log`), and that the primary metric is `private_working_set`. A GPU source is not needed on this build: `GPU source none` is the expected line. Fix whatever is wrong in the harness code.
...
4. Short smoke run first, to validate discovery and metrics on the real app without long phases:
   `uv run erudi_eval.py run --flavour win-cpu --phases preflight,cold_boot,idle_after_boot,ui_tour,quit --idle-seconds 30`
...
5. **Two full runs, one per profile** (the point is to compare a clean machine with a realistic one):
   - **clean**: reboot Windows, open only the terminal you run the harness from, then
     `uv run erudi_eval.py run --flavour win-cpu --profile clean --cleanup`.
   - **nominal**, right after, without rebooting: open Chrome with 4 tabs (Gmail, Google Docs, a paused YouTube video, GitHub), the chat apps idle, Windows Terminal with 5 coding-agent CLI sessions open and idle for the whole run, then
     `uv run erudi_eval.py run --flavour win-cpu --profile nominal --cleanup`.
   ...
   A CPU run is slower than a GPU one: allow well over an hour per full run, and do not touch the machine while it runs.
6. In the report of each full run, read the **Flavour and model** section and check:
   - the running app confirmed `win-cpu` (backend_type `cpu`, engine `CPU_Engine`);
   - the inference process was seen as `llama_server` and the `inference` category contains `llama-server.exe`;
   - `-ngl` is `0` (nothing offloaded: this is the CPU build) and `--threads` matches the machine's core count;
   - nothing claims GPU memory: the per-process GPU column stays empty and the flavour line says `none`.
...
7. If a phase fails, read `events.jsonl`, `phases.json` and the copied logs in `results/<run>/logs/`, decide whether it is a harness bug (fix and re-run that phase with `--phases`) or an app behaviour (record it, do not "fix" the app). On a slow CPU machine a phase can hit a timeout that is generous on a GPU machine: say so instead of shortening the scenario.

## What to bring back

1. A zip of the `results/` folder (all runs, including failed ones).
2. A patch of your harness changes. Before editing anything, copy the untouched folder to `..\erudi-eval-original`; at the end produce `git diff --no-index ..\erudi-eval-original . > windows-cpu-fixes.patch` (excluding `results/`, `.venv/`, `__pycache__/`), and list each change with its reason (file, what was wrong on Windows, what you changed).
3. A short report in French:
   - machine: CPU model and core count, RAM, Windows version, Erudi version, flavour `win-cpu`;
   - for both profiles, the conditions printed in the report header (uptime, swap at start, power) and the headline numbers from `report.md`: app overhead (everything except inference) at idle after boot, during chat and after KB ingestion; inference memory during chat; boot duration; TTFT cold and warm; `generation s` and characters per second; install size and data root size;
