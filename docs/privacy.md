...
- **The inference server's own output**, on Apple Silicon only: `mlx-child-<port>.log`, next to the backend log. It holds what `mlx_vlm.server` prints — the model it loaded, one line per request with its token counts and speed, and whatever it fails on — not the text of your messages ([mlx_child_log.py](https://github.com/erudi-app/erudi/blob/main/backend/src/engines/mlx_child_log.py)). It is deleted when that model is unloaded, and kept when the server dies on its own, so the crash can be read afterwards.
...
None at the moment. When the build is weaker than this page would like it to be, the point is listed here and tracked as an issue, so that you do not have to find it yourself.
