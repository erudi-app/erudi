...
A local model goes quiet for two unrelated reasons, so each model call carries two
wall-clock budgets instead of langchain-openai's single `stream_chunk_timeout` (which
the client switches off):

| Silence | Budget | Why |
|---|---|---|
| Before the first chunk | `clamp(120 s, 30 s + estimated prompt tokens / 25, ceiling)` | The model is reading the prompt. That grows with the conversation: 6878 tokens prefill in 132.9 s on an Apple M4 base running a 9B model, and a CPU-only machine is slower still. |
| Between chunks | 120 s | Decode emits a token every few milliseconds on every engine, so two minutes of silence is a hang. |

The ceiling derives from the loaded child's allocated context window
(`effective_context_tokens`, stamped on the client per turn by the model factory):
`max(900 s, 30 s + window / 25)`, so a legitimately full window is never killed
mid-prefill — with the dynamic context window, a flat 900 s would recreate the very
kill the two budgets fixed. An unknown window keeps 900 s.
...
histories reach the ceiling; that only delays the detection of a true hang. Both
...
cause: the model never started on a prompt this size, or it started and went silent.
