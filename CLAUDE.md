...
Hardware backend is selected at startup by `BaseEngine.get_engine()` (`backend/src/engines/base_engine.py`): `MLX_Engine` on macOS ARM, `CUDA_Engine` on Linux/Windows with NVIDIA, `CPU_Engine` otherwise. Set `ERUDI_FORCE_CPU=1` to bypass GPU detection. Detection is not final: after the migrations the lifespan applies `user_settings.inference_backend` (`auto` / `cpu`), swapping `CUDA_Engine` for `CPU_Engine` when the user pinned the processor — safe because both carry `FORMAT_TAG = "gguf"` — and then runs a CUDA pre-flight (`src/engines/cuda_compatibility.py`) that emits an `engine_notice` event when the GPU or its driver is below what the bundled CUDA build needs. The fallback is proposed, never applied automatically; `ERUDI_FORCE_CPU` wins over the setting. Every environment variable the backend reads is listed in `backend/.env.example` (the only secret is `HF_TOKEN`, read from the environment at runtime and never embedded in a build); `tests/test_env_example.py` fails on any new `os.getenv` that is not listed there.
...
Routers mounted under `/erudi` (in `core/api.py:register_routers`): `llms`, `hardware`, `arena`, `knowledge_base`, `conversations`, `startup`, `user_settings`, `diagnostics` (from `domains/`) plus `health` (from `core/health.py`, not a domain). The frontend hits `http://127.0.0.1:27182/erudi/...` (see `frontend/src/config/api.js`).
...
- **Error logging follows `docs/logging.md`** ("Logging rules"): one record per failure, written where it is handled (the request handler, the task boundary), at the level of what happened — `ERROR` with the traceback when the operation did not happen, `WARNING` when the app degraded on its own, `INFO` for a 4xx the client asked for.
  No silent `except`/`catch` without a comment saying why; a parent logs its children's deaths (inference child, backend, renderer); secrets (`HF_TOKEN`, the llama-server `--api-key`, database URLs) never reach a log line.
- **ASCII-only where it executes or gets parsed**: log-message literals (`logger.*(...)` strings) and machine-parsed bundled data files (e.g. `alembic.ini`) must stay ASCII-only — no `→`, `—`, accents. Non-ASCII is fine in comments and docstrings (Python reads source as UTF-8 regardless of locale).
- **Async-first.** Don't block the event loop with synchronous I/O in endpoints/services.
- **Ruff config** (`backend/ruff.toml`) only enforces `F` + `E7`. `E501`/`E402`/`F841`/`E701` are intentionally ignored — don't reintroduce them as blockers. `ruff format` is the project's only formatter, and `backend/ruff.toml` sets `line-length = 100` at the top level so the formatter and the linter agree without CLI flags. The root `ruff.toml` carries the same width for the Python outside `backend/` (`scripts/ci/smoke_boot.py` and the dev-tooling modules in `scripts/dev/`); ruff picks the nearest config per file and never merges the two. `ruff format` runs from `.pre-commit-config.yaml` and is checked in CI — never hand-format around it.
...
- **Documentation moves with the code, in the same change** — see `CONTRIBUTING.md` (Documentation).
- **Documentation is present-tense** — see `CONTRIBUTING.md` (Documentation).
- **Commits**: `type(scope): description` (`feat`, `fix`, `docs`, `chore`, `ci`), carrying no attribution trailers.
- **Branches and pushes**: see `CONTRIBUTING.md` (Git) — squash-merged through a protected, must-be-up-to-date `main`, so never rebase/amend a pushed branch and don't groom branch history.
...
- **Embedded PostgreSQL + pgvector** via `pgserver` (pip wheels, no Docker, no system install). The FastAPI lifespan boots the cluster (`src/launcher/postgres_runtime.py`, data dir `backend/data/postgres/` in dev; user-writable dir via `runtime_paths.py` in packaged builds), creates the `erudi` database + `vector` extension, then binds SQLAlchemy through `init_database(url)` (psycopg3, `postgresql+psycopg://`). Every `start_postgres` also enforces a per-cluster password: generated once into `<data_dir>/erudi_db_password`, set on the `postgres` role, required by every `host` rule of `pg_hba.conf` (`scram-sha-256`; `local` socket rules stay `trust`), and carried by both handle URLs — never log a URL unmasked, and never put it on a command line (`backup.py` hands it to `pg_dump` via `PGPASSWORD`). Never import `db_engine` by value — read it via `database.core` attributes after init.
...
- **Backend** (`.github/workflows/backend-ci.yml`, Python 3.12, 3-leg matrix): `compileall`, `ruff check backend scripts`, `ruff format --check backend scripts`, `from src.main import app`, `pytest tests/ -q --ignore=tests/e2e -m "not mlx_only"`. All three legs (`ubuntu-latest`, `windows-latest`, `macos-14`) gate merges and run with `-x`. `pytest.ini` sets `timeout = 600` so a hung test fails instead of consuming the job. The Ubuntu leg runs against `CPU_Engine` only — keep CPU paths working.
- **Frontend** (`.github/workflows/frontend-ci.yml`, Node 20 — `webpack-cli` needs >= 20.9, which the CI runner's Node 20 satisfies): `npm ci`, `npm run lint:check`, `npm run format:check`, `npm run test:run` (Vitest).
