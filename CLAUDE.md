...
- **Embedded PostgreSQL + pgvector** via `pgserver` (pip wheels, no Docker, no system install). The FastAPI lifespan boots the cluster (`src/launcher/postgres_runtime.py`, data dir `backend/data/postgres/` in dev; user-writable dir via `runtime_paths.py` in packaged builds), creates the `erudi` database + `vector` extension, then binds SQLAlchemy through `init_database(url)` (psycopg3, `postgresql+psycopg://`). Every `start_postgres` also enforces a per-cluster password (#462): generated once into `<data_dir>/erudi_db_password`, set on the `postgres` role, required by every `host` rule of `pg_hba.conf` (`scram-sha-256`; `local` socket rules stay `trust`), and carried by both handle URLs — never log a URL unmasked, and never put it on a command line (`backup.py` hands it to `pg_dump` via `PGPASSWORD`). Never import `db_engine` by value — read it via `database.core` attributes after init.
